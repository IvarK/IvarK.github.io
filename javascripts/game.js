...
function getTimeDimensionPower(tier) {
    ...
    if (ECTimesCompleted("eterc10") !== 0) ec10bonus = new Decimal(Math.max(Math.pow(getInfinitied(), 0.9) * ECTimesCompleted("eterc10") * 0.000002+1, 1)).pow(4)
...
function eternity(force) {
    if ((player.infinityPoints.gte(Number.MAX_VALUE) && (!player.options.eternityconfirm || confirm("Eternity will reset everything except achievements and challenge records. You will also gain an Eternity point and unlock various upgrades."))) || force === true) {
        ...
        if (player.dilation.active && !force) {
            let tachyonGain = Math.max(Math.pow((Decimal.log10(player.money) / Decimal.log10(1000))/1000, 1.5) * (Math.pow(3, player.dilation.rebuyables[3])) - player.dilation.totalTachyonParticles, 0)
 ...
 */
 ...
 const DIL_UPG_COSTS = [null, [1e4, 20], [1e6, 1000], [1e7, 30], 
                              9e99,        9e99,         9e99,
                              9e99,        9e99,         9e99]
