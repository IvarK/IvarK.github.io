...
function formatValue(notation, value, places, placesUnder1000) {
    if ((value != Infinity) && (value >= 1000)) {
        ...
        if ((notation === "Standard") && (((power - (power % 3)) / 3) < FormatList.length - 1)) {
            ...
            if ((Math.floor(((power - (power % 3)) / 3) / letterList.length) < 1)) {
                return ((Math.round(matissa * Math.pow(10, power % 3) * Math.pow(10, places)) / Math.pow(10, places)).toFixed(places) + letterList[((power - (power % 3)) / 3) % letterList.length]);
            } else {
                return ((Math.round(matissa * Math.pow(10, power % 3) * Math.pow(10, places)) / Math.pow(10, places)).toFixed(places) + letterList[Math.floor(((power - (power % 3)) / 3) / letterList.length) - 1] + letterList[((power - (power % 3)) / 3) % letterList.length]);
            }
        } else {
...
function setAchieveTooltip() {
    ...
    var noNerf = document.getElementById("I forgot to nerf that");
    ...
    noNerf.setAttribute('ach-tooltip', "Get any Dimension multiplier over" + formatValue(player.options.notation, 1e31, 0, 0));
}
