...
function onLoad() {
    ...
    if (player.autoCrunchMode === undefined) player.autoCrunchMode = false
...
function getGalaxyRequirement() {
    ...
    if (player.timestudy.studies.includes(42)) amount = 80 + (player.galaxies * 58)
...
function updateDimensions() {
    ...
    if (player.timestudy.studies.includes(31)) document.getElementById("postinfi12").innerHTML = "Power up all dimensions based on amount infinitied <br>Currently: "+shortenMoney(Math.pow((Math.log10(player.infinitied)*10).toFixed(2), 4))+"x<br>Cost: "+shortenCosts(1e5)+" IP"
...
function getInfinityDimensionPower(tier) {
    ...
    if (player.replicanti.unl && player.replicanti.amount > 1) {
        ...
        else mult = mult.times(Math.pow(player.replicanti.amount, 0.04) +(Math.pow(Math.log2(player.replicanti.amount), 2)))
    }
...
var studyCosts = [1, 3, 2, 4, 2, 5, 7, 7, 2, 71, 72, 73, 81, 82 ,83, 91, 92, 93, 101, 102, 103, 111, 121, 122, 131, 132, 141, 142, 151, 152, 161, 171]
...
function softReset(bulk) {
    player.resets+=bulk;
    if (bulk >= 1000) giveAchievement("96");
    player = {
        money: new Decimal(10),
        tickSpeedCost: new Decimal(1000),
        tickspeed: new Decimal(1000),
...
function giveAchievement(name) {
    ...
    if (name == "All your IP are belong to us" || name == "93") {
...
function updateInfCosts() {
    ...
    document.getElementById("replicantiinterval").innerHTML = "Interval: "+(player.replicanti.interval).toFixed(places)+"ms<br>-> "+(player.replicanti.interval*0.9).toFixed(places)+" Costs: "+shortenCosts(player.replicanti.intervalCost)+" IP"
    ...
    document.getElementById("replicantiinterval").className = (player.infinityPoints.gte(player.replicanti.intervalCost) && (player.replicanti.interval !== 50) || player.timestudy.studies.includes(22)) ? "storebtn" : "unavailablebtn"
...
function upgradeReplicantiInterval() {
    if (player.infinityPoints.gte(player.replicanti.intervalCost) && (player.replicanti.interval > 50 || player.timestudy.studies.includes(22))) {
...
function toggleCrunchMode() {
    if (player.autoCrunchMode) {
        player.autoCrunchMode = false
        document.getElementById("togglecrunchmode").innerHTML = "Auto crunch mode: amount"
    } else {
        player.autoCrunchMode = true
...
function updateLastTenRuns() {
    ...
    if (tempBest >= 1e300) giveAchievement("93");
...
setInterval(function () {
    ...
    if (player.infinityPower > 1e60) giveAchievement("94"); //TBD
    ...
    if (current == 1024 && player.thisInfinityTime < 600*30) giveAchievement("95")
    ...
    if (player.timestudy.studies.includes(21)) document.getElementById("replicantimult").innerHTML = shorten(Math.pow(player.replicanti.amount, 0.04) +(Math.pow(Math.log2(player.replicanti.amount), 2)))
...
setInterval( function() {
    ...
    currentMult = Math.pow(player.money.e+1, 0.5)
    infinitiedMult = Math.log10(player.infinitied)*10
    if (player.timestudy.studies.includes(31)) infinitiedMult = Math.pow(Math.log10(player.infinitied)*10, 4)
    achievementMult = Math.max(Math.pow((player.achievements.length-30), 3)/40,1)
    challengeMult = Decimal.max(10*3000/worstChallengeTime, 1)
    unspentBonus = Decimal.pow(player.infinityPoints.dividedBy(2),1.5).plus(1)
}, 500)
