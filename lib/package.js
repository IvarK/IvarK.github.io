...
function Game() {
    ...
    for (var _i2 = 1; _i2 <= 8; ++_i2) {
        this.dimensions[_i2] = new Dimension(_i2);
    }
...
function softReset(props) {
    ...
    for (var _i3 = 2; _i3 <= 8; ++_i3) {
        game.dimensions[_i3].dom.row.style.display = "none";
    }
...
shorten = function shorten(money) {
    return formatValue(player.options.notation, money, 2, 2);
};

shortenCosts = function shortenCosts(money) {
    return formatValue(player.options.notation, money, 0, 0);
};

shortenDimensions = function shortenDimensions(money) {
    return formatValue(player.options.notation, money, 2, 0);
};

shortenMoney = function shortenMoney(money) {
...
document.getElementById("reset").onclick = function () {
    if (confirm("Do you really want to erase all your progress?")) {
        ...
        for (var _i4 = 2; _i4 <= 8; ++_i4) {
            game.dimensions[_i4].dom.row.style.display = "none";
        }
